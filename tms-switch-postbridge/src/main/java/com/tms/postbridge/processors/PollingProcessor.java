...
@Component
public class PollingProcessor implements PostBridgeSinkTransactionProcessor {

    public static TransactionRequest createPollingMessage(StanProvider stanProvider) {
        Date now = new Date();

        return TransactionRequest.builder()
                .requestType(RequestType.POLLING)
    ...
    public boolean canConvert(RequestType requestType) {
        return RequestType.POLLING.equals(requestType);
    }
    ...
    public ISOMsg toISOMsg(TransactionRequest transactionRequest) throws TransactionProcessingException {
        ...
        try {
            ...
            String msg = String.format("There was a channel error converting postbridge polling message ex: %s", e.toString());
